...
def search_word(word):
    ...
    if soup.find(text="Aviso: ") or related_message:
        ...
        for k, entry in enumerate(soup.select("#resultados > article"), start=1):
            ...
            if longitude == len(expressions_name):
                for a in range(longitude):
                    val_meaning[expressions_name[a].text] = expressions_info[a].text
            else:
                a = -1
                for info_entry in expressions_info:
                    ...
                    if info_text[0] in {"1", 'V'}:
                        a += 1
                        val_meaning[expressions_name[a].text] = [info_text]
                    else:
                        val_meaning[expressions_name[a].text].append(info_text)
        ...
        if dict_meaning:
            ...
            print(f'\nNo se ha encontrado información sobre la palabra "{word}"')
